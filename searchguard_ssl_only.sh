...
do_install() {
  ...
  dolog "Will bootstrap $STACKNAME in $REGION on $SG_PUBHOST ($DIST) SSL only"
  ...
  $ES_BIN/elasticsearch-plugin install -b com.floragunn:search-guard-ssl:$SG_VERSION > /dev/null 
  ...
  curl -XPUT -k "https://$SG_PUBHOST:9200/twitter/tweet/1?pretty" -d'
  {
    "user" : "searchguard",
  ...
  curl -XPUT -k "https://$SG_PUBHOST:9200/twitter1/tweet/1?pretty" -d'
  {
    "user" : "searchguard1",
  ...
  curl -ksS "https://$SG_PUBHOST:9200/_cluster/health?pretty" > health 2>&1
  check_ret "final curl 1"
  curl -ksS "https://$SG_PUBHOST:9200/_searchguard/authinfo?pretty" > authinfo 2>&1
  check_ret "final curl 2"
  curl -ksS "https://$SG_PUBHOST:9200/_searchguard/sslinfo?pretty" > sslinfo 2>&1
